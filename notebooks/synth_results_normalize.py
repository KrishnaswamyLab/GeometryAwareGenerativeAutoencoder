...
for i in tqdm(range(len(sweep.runs))):
    ...
    dist_std = 1.
    input_dim = 100
    model = make_model(cfg, input_dim, emb_dim, pp, dist_std, None, None, from_checkpoint=True, checkpoint_path=ckpt_path)
...
res_df.to_csv("synth_results_w_recon_normalize.csv", index=False)
...
rounded_res_df.to_csv("synth_results_w_recon_normalize_rounded.csv", index=False)