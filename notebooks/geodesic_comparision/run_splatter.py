...
splited_name = cfg_main_data_name.split("_")
...
start_idx, end_idx = data_gt['pairs'].T
# In[94]:
...
np.savez(f"{res_path}/ours.npz", geodesic_lengths=length2.detach().cpu().numpy(), geodesics=geods.detach().cpu().numpy())
...
#     dist2geod=dist2geod,
# ))
np.savez(f"{res_path}/no_density.npz", geodesic_lengths=length2.detach().cpu().numpy(), geodesics=geods.detach().cpu().numpy())
...
#     dist2geod=dist2geod,
# ))
np.savez(f"{res_path}/density.npz", geodesic_lengths=length2.detach().cpu().numpy(), geodesics=geods.detach().cpu().numpy())
