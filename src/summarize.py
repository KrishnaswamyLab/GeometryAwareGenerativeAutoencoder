...
def summarize_metrics(prob_methods, loss_types, alphas, knns, data_names, data_noises, seeds):
    result = []
                ...
                for knn in knns:
                    for data_name in data_names:
                        for data_noise in data_noises:
                            for seed in seeds:
                                path = f'../results/sepa_{prob_method}_{loss_type}_a{alpha}_knn{knn}_{data_name}_noise{data_noise:.1f}_seed{seed}'
...
def main():
    ...
    metrics = summarize_metrics(prob_methods, loss_types, alphas, knns, data_names, data_noises, seeds)
    ...
    metrics = metrics.groupby(['prob_method', 'loss_type', 'alpha', 'knn', 'data_name', 'data_noise']).agg(['mean', 'std'])
